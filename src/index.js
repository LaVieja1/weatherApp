(function displayWeather() {
    ...
    async function getWeather(input) {
      ...
      try {
        const response = await fetch(
          `https://api.openweathermap.org/data/2.5/weather?${searchTerm}&appid=0c9da8e756d3317486decdc48e6175c0&units=standard`,
          { mode: 'cors' }
        );
    ...
    function displayWeather(data) {
      ...
      tempFeeling.innerHTML = `${'Se siente como: '}${tempFeel + '&deg'}`;
  
      humidityMeter.innerHTML = `${'Nivel de humedad: '}${
        data.main.humidity + '%'
      }`;
      windMeter.innerHTML = `${'Viento: '}${data.wind.speed + 'k/m'}`;
